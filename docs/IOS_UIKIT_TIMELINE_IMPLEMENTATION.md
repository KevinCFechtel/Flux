...
| Article accessory geometry. | Unread is always outermost/topmost, followed by Star, Comments, then Audio with optional duration. Visual portrait uses a leading 85% hero with the vertical rail attached directly beside it, aligns Date beneath the feed title with compact metadata spacing, and keeps Preview full width; Compact, Visual compact and Visual landscape keep the corresponding horizontal order. Audio stays unrendered until it is supplied by a batched Timeline projection; never fetch enclosure state from each visible cell. |
