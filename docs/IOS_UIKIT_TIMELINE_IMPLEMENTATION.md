...
| Article accessory/info geometry. | Unread is always outermost/topmost, followed by Star, Comments, then Audio with optional duration. For the iOS 27 A/B validation, Visual portrait defaults to the classic 100% content-width hero followed by metadata, title, and the unchanged absolute-date/trailing-reading-time row. Developer Diagnostics can switch only Visual portrait back to the retained leading 80% hero + attached info rail, where status remains top-aligned and optional Miniflux reading time sits above frozen relative publication age at the bottom. The A/B mode is part of deterministic layout and Timeline geometry identity, so row heights and image target sizes cannot alias across variants. Compact, Visual compact, and Visual landscape keep their existing geometry and date-row behavior. Reading time comes through persisted Core/UniFFI article metadata, never a per-cell calculation/query. Visual compact's title remains an explicit non-collapsing row between metadata and the date/image row. Audio stays unrendered until supplied by a batched Timeline projection. |
