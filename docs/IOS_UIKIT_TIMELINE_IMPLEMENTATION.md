...
| Article accessory/info geometry. | Unread is always outermost/topmost, followed by Star, Comments, then Audio with optional duration. Visual portrait uses a leading 80% hero with a directly attached info rail: status slots stay top-aligned while optional Miniflux reading time with a document/text glyph sits above frozen relative publication age with a clock glyph; the publication-age line is bottom-aligned to the hero. Its absolute date is removed from the leading stack; Preview remains full width. The age string is prepared once per structural Timeline generation and paging reuses the same reference date, so no timer or visible-row update exists. Reading time comes through persisted Core/UniFFI article metadata, never a per-cell calculation/query. Compact, Visual compact, and Visual landscape keep the date row height unchanged and use otherwise-free trailing space for optional `doc.text` + Miniflux reading time; the date label yields horizontal width but no new vertical line is introduced. Visual compact's title remains an explicit non-collapsing row between metadata and the date/image row. Audio stays unrendered until supplied by a batched Timeline projection. |
