...
| Symbol | Class | Reason |
|---|---|---|
| `IOSUIKitTimelineTopScrimView` | Promote | The static status-bar gradient remains shipping behaviour below iOS 27. iOS 27 instead runs a bounded real-device trial of the native `UITableView.topEdgeEffect` with the system `.automatic` style; iOS 26 keeps native edge effects disabled because the measured progressive blur hurt scrolling. |
