...
The default iOS/iPadOS Article List actions are Sync, Filter/Sort, and More. iPhone portrait exposes them in the Bottom Action Bar using a structurally distinct bottom-toolbar branch; its scope capsule is centered in an independent top safe-area inset. iPhone landscape renders the compact scope capsule on the leading side and one detached floating Sync/Filter/More capsule on the trailing side of that same inset row. When persistent split navigation is visible, iPadOS shows only the detached action capsule because the sidebar already communicates the selected scope; if iPadOS temporarily hides that sidebar, the detail view restores the scope capsule with chevron on the leading side. The Article List's otherwise-empty top navigation bar is hidden in every mode. These custom controls remain outside native scroll-edge geometry while the UIKit Timeline still uses the system `.automatic` top edge effect on iOS/iPadOS 26+. In the sidebar, category selection and category expansion are separate interactions so expanding or collapsing a selected category does not depend on List-selection behavior. This Apple layout is not an Android requirement.
