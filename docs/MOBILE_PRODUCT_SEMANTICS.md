...
On native iOS/iPadOS, standard Visual portrait uses the performance-motivated 80% hero as a leading main-content column. The trailing rail begins directly beside the hero. Its upper section preserves the stable status order, while the temporal information block is anchored to the hero's lower edge: reading time with a text/document symbol when Miniflux supplies it, then article age with a clock symbol as the bottom-most line. The publication age is derived once from `published_at` when a structural Timeline generation is prepared and remains immutable for that generation; paging reuses the same reference time and no visible-row timer refreshes it. Miniflux `reading_time` is persisted and projected through Core/UniFFI rather than calculated by the iOS cell. Visual portrait removes the absolute publication date from the leading text column. Visual landscape, Compact, and Visual compact retain the absolute date and, when Miniflux supplies reading time, place a trailing `doc.text` + duration in that same date row without adding row height. Visual compact must retain its article title between metadata and the date/image row; its title may never collapse merely because the date row owns a trailing reading-time item. The preview remains full content width so its wrapping behavior stays independent from the 80/20 hero composition. Audio is added only when the Timeline receives a batched article-level audio projection; list cells must not perform per-row enclosure/media fetches merely to populate the accessory.
...
The default iOS/iPadOS Article List actions are Sync, Filter/Sort, and More. iPhone portrait exposes them in the Bottom Action Bar using a structurally distinct bottom-toolbar branch. iPhone landscape moves the same actions to a structurally distinct trailing top toolbar and keeps an interactive compact leading title capsule with the current-scope article count in compact numeric form. When persistent split navigation is visible, iPadOS also uses the trailing top toolbar and leaves the detail navigation bar title-free because the sidebar already communicates the selected scope. If iPadOS temporarily hides that sidebar, the detail view restores an explicit leading scope capsule with chevron so navigation is discoverable without relying on an edge swipe. The leading toolbar slot remains structurally present while the sidebar is visible and the capsule is only hidden/non-interactive, preventing sidebar transitions from rebuilding navigation chrome and disturbing the article timeline. In the sidebar, category selection and category expansion are separate interactions so expanding or collapsing a selected category does not depend on List-selection behavior. This Apple layout is not an Android requirement.
