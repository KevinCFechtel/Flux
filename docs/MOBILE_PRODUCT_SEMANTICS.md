...
The default iOS/iPadOS Article List actions are Sync, Filter/Sort, and More. iPhone portrait exposes them in the Bottom Action Bar using a structurally distinct bottom-toolbar branch; its scope capsule is centered in an independent top safe-area inset and the otherwise-empty top navigation bar is hidden. iPhone landscape moves the actions to a structurally distinct trailing top toolbar while the interactive compact scope capsule sits in a separate leading-aligned inset row below that system bar. When persistent split navigation is visible, iPadOS also uses the trailing top toolbar and leaves the detail title-free because the sidebar already communicates the selected scope. If iPadOS temporarily hides that sidebar, the detail view restores an explicit scope capsule with chevron in the detached inset row so navigation is discoverable without relying on an edge swipe. This separation keeps the custom capsule out of native scroll-edge geometry; iOS/iPadOS 26+ may therefore use the system automatic top edge effect for actual system chrome. In the sidebar, category selection and category expansion are separate interactions so expanding or collapsing a selected category does not depend on List-selection behavior. This Apple layout is not an Android requirement.
