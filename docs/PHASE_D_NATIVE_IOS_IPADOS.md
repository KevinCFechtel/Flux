...
Article-row accessories use one stable semantic order: **Unread → Star → Comments → Audio**, measured outermost-to-innermost for horizontal groups and top-to-bottom for a vertical rail. Optional listening duration belongs to the Audio accessory. Standard Visual portrait turns the existing 80% hero allocation into an intentional leading content column. The rail begins immediately beside that column rather than being centered in the remaining gutter. The status slots remain top-aligned; the frozen relative publication age (`clock.arrow.circlepath`) and Miniflux reading duration when available (`doc.text`) form a bottom-aligned temporal group whose last visible line meets the hero's lower edge. Visual portrait therefore uses **Title → Feed → Hero/Info Rail → Preview** and omits the absolute date from the leading text stack. The relative age is created once per structural Timeline generation from one shared reference date; later keyset pages reuse that same reference and visible rows never run timers or mutate their age string. Miniflux `reading_time` is a persisted Core article field (schema v18) projected through `ArticleSummary`/UniFFI; the iOS Timeline never calculates reading duration itself. The preview remains full content width. Compact, Visual compact, and Visual landscape retain their existing absolute-date layouts and horizontal accessory ordering. The current Timeline projection already supplies unread/star/comments; Audio must join through a batched article-level Timeline/media projection before its slot becomes productive. Do not introduce visible-cell enclosure lookups or other per-row Core calls to populate Audio or duration.
