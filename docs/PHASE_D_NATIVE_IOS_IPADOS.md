...
Article-row accessories use one stable semantic order: **Unread → Star → Comments → Audio**, measured outermost-to-innermost for horizontal groups and top-to-bottom for a vertical rail. Optional listening duration belongs to the Audio accessory. During the iOS 27 real-device performance comparison, standard Visual portrait has an explicit A/B layout seam. The default comparison path restores the classic **100% content-width hero → metadata → title → absolute date + trailing reading time → preview** ordering. Developer Diagnostics can opt back into the retained **80% hero + info rail** path without affecting Compact, Visual compact, or wide Visual geometry. The 80/20 path keeps status slots top-aligned and Miniflux reading duration (`doc.text`) above frozen relative publication age (`clock.arrow.circlepath`) in its bottom-aligned rail, with the preview still full content width. Switching modes is part of Timeline geometry identity so row-height preparation and article-image request sizing cannot alias across the A/B variants. Miniflux `reading_time` remains a persisted Core article field (schema v18) projected through `ArticleSummary`/UniFFI; the iOS Timeline never calculates reading duration itself. Compact, Visual compact, Visual landscape, and the classic 100% portrait path keep the absolute date on the leading side of their date row and project Miniflux reading time, when available, as a trailing `doc.text` + duration in that same row; this is a horizontal-only addition and must not increase deterministic text-row height. Visual compact keeps its metadata-first ordering and its article title between metadata and date/image content. The current Timeline projection already supplies unread/star/comments; Audio must join through a batched article-level Timeline/media projection before its slot becomes productive. Do not introduce visible-cell enclosure lookups or other per-row Core calls to populate Audio or duration.
