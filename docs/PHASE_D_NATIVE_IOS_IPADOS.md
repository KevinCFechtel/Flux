...
Article-row accessories use one stable semantic order: **Unread → Star → Comments → Audio**, measured outermost-to-innermost for horizontal groups and top-to-bottom for a vertical rail. Optional listening duration belongs to the Audio accessory. Standard Visual portrait turns the existing 85% hero allocation into an intentional leading content column: the hero uses that width while the remaining trailing gutter carries the vertical accessory rail. The preview remains full content width to preserve its historical wrapping and deterministic row-height semantics. Compact, Visual compact, and Visual landscape remain horizontal and preserve the same semantic ordering. The current Timeline projection already supplies unread/star/comments; Audio must join through a batched article-level Timeline/media projection before its slot becomes productive. Do not introduce visible-cell enclosure lookups or other per-row Core calls to populate Audio or duration.
