...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if let button = statusItem.button { button.image = NSImage(systemSymbolName: "dot.radiowaves.left.and.right", accessibilityDescription: "FluxBar"); button.image?.isTemplate = true; button.imagePosition = .imageLeading; button.target = self; button.action = #selector(togglePopover); button.toolTip = "FluxBar" }
    ...
    private func size(sidebarVisible: Bool) -> NSSize { NSSize(width: PopoverLayout.articleWidth + (sidebarVisible ? PopoverLayout.sidebarWidth : 0), height: min(PopoverLayout.height, (statusItem.button?.window?.screen?.visibleFrame.height ?? PopoverLayout.height + 32) - 32)) }
}
