...
enum SystemNotificationPresentation {
    ...
    static func body(
        ...
        let count = newCount == 1
            ...
            : String(format: String(localized: "%lld new articles", bundle: bundle, locale: locale), locale: locale, newCount)
        let formatter = DateFormatter()
        formatter.locale = locale
        formatter.timeZone = timeZone
        formatter.dateStyle = .short
        formatter.timeStyle = .short
        return String(format: String(localized: "%@ · %@", bundle: bundle, locale: locale), locale: locale, count, formatter.string(from: submittedAt))
    }
}
